RPA1 = ['RECIFE', 'BOA VISTA', 'CABANGA',
        'COELHOS', 'ILHA DO LEITE', "ILHA JOANA BEZERRA",
        'PAISSANDU', 'SANTO AMARO', 'SANTO ANTONIO', 'SAO JOSE', 'SOLEDADE']

RPA2 = ['AGUA FRIA', 'ALTO SANTA TEREZINHA','TORREAO',
        'ARRUDA', 'BEBERIBE', 'BOMBA DO HEMETERIO', 'CAJUEIRO',
        'CAMPINA DO BARRETO', 'CAMPO GRANDE', 'DOIS UNIDOS', 'ENCRUZILHADA',
        'FUNDAO', 'HIPODROMO', 'LINHA DO TIRO', 'PEIXINHOS', 'PONTO DE PARADA', 'PORTO DA MADEIRA', 'ROSARINHO']

RPA3 = ['AFLITOS', 'ALTO DO MANDU', 'ALTO JOSE DO PINHO', 'ALTO JOSE BONIFACIO',
         'APIPUCOS', 'BREJO DA GUABIRABA', 'BREJO DE BEBERIBE','CASA AMARELA', 'CASA FORTE',
        'CORREGO DO JENIPAPO', 'DERBY', 'DOIS IRMAOS', 'ESPINHEIRO', 'GRACAS', 'GUABIRABA',
        'JAQUEIRA', 'MACAXEIRA', 'MANGABEIRA', 'MONTEIRO','MORRO DA CONCEICAO', 'NOVA DESCOBERTA',
        'PARNAMIRIM', 'PASSARINHO', 'PAU-FERRO',  'POCO', 'SANTANA',' SITIO DOS PINTOS - SAO BRAS',
        'TAMARINEIRA','VASCO DA GAMA', 'CAPUNGA']

RPA4 = ['CAXANGA','CIDADE UNIVERSITARIA',
        'CORDEIRO','ENGENHO DO MEIO', 'ILHA DO RETIRO', 'IPUTINGA', 
        'MADALENA', 'PRADO', 'TORROES', 'TORRE', 'VARZEA', 'ZUMBI']

RPA5 = ['AFOGADOS', 'AREIAS', 'BARRO','TEJIPIO',
        'BONGI', 'CACOTE', 'COQUEIRAL', 'CURADO','ESTANCIA',
        'JARDIM SAO PAULO', 'JIQUIA', 'MANGUEIRA', 'MUSTARDINHA',
        'SAN MARTIN', 'SANCHO', 'TOTO']

RPA6 = ['BOA VIAGEM', 'BRASILIA TEIMOSA', 'IBURA DE BAIXO', 'JORDAO', 'TRES CARNEIROS',
        "COHAB", 'IBURA', "IMBIRIBEIRA", 'IPSEP', 'JORDAO ALTO', 'JORDAO BAIXO', "PINA"]

SCHEMA_TIPOS = {
    'secao': 'str',
    'zona': 'str',
    'CD_Local': 'int64',
    'votos_recebidos': 'float64',
    'votos_nominais': 'float64',
    'votos_legenda': 'float64',
    'votos_totais': 'float64',
    'votos_partido': 'float64',
    'local': 'str',
    'endereco': 'str',
    'EBAIRRNOMEOF': 'str',
    'nome_candidato': 'str',
    'nome_urna': 'str',
    'sigla_partido': 'str',
    'RPA': 'str'
}

vencedores = [
'Aderaldo Pinto',
'Agora é Rubem',
'Alcides Teixeira Neto',
'Alef Collins',
'Professora Ana Lúcia',
'Carlos Muniz',
'Chico Kiko',
'Cida Pedrosa',
'Davi Muniz',
'Eduardo Mota',
'Eduardo Moura',
'Eriberto Rafael',
'Fabiano Ferraz',
'Felipe Alecrim',
'Felipe Francismar',
'Flávia de Nadegi',
'Fred Ferreira',
'Gilberto Alves',
'Gilson Machado Filho',
'Hélio Guabiraba',
'Júnior Bocão',
'Jô Cavalcanti',
'Zé Neto',
'Júnior de Cleto',
'Kari Santos',
'Liana Cirne',
'Luiz Eustáquio',
'Natália de Menudo',
'Osmar Ricardo',
'Paulo Muniz',
'Rinaldo Júnior',
'Rodrigo Coutinho',
'Romerinho Jatobá',
'Samuel Salazar',
'Tadeu Calheiros',
'Thiago Medina',
'Wilton Brito',
]

bairros = {
    'IMBIRIBEIRA': 'IMBIRIBEIRA',
    'BRASILIA TEIMOSA': 'BRASILIA TEIMOSA',
    'BOA VIAGEM': 'BOA VIAGEM',
    'PINA': 'PINA',
    'IPSEP': 'IPSEP',
    'CABANGA': 'CABANGA',
    'AREIAS': 'AREIAS',
    'JARDIM SÃO PAULO': 'JARDIM SAO PAULO',
    'ESTÂNCIA': 'ESTANCIA',
    'AFOGADOS': 'AFOGADOS',
    'JARDIM UCHÔA': 'JAQUEIRA',
    'TEJIPIÓ': 'TEJIPIO',
    'BARRO': 'BARRO',
    'IBURA': 'IBURA',
    'COHAB': 'COHAB',
    'UR 1 - IBURA': 'IBURA',
    'MONTE VERDE - IBURA': 'IBURA',
    'LAGOA ENCANTADA': 'IBURA',  
    'UR-4': 'IBURA',
    'UR 3 - IBURA': 'IBURA',
    'TOTÓ': 'TOTO',
    'UR 2 - IBURA ALTO': 'IBURA',
    'TRES CARNEIROS': 'IBURA',  
    'SANCHO': 'SANCHO',
    'SAO JOSE': 'SAO JOSE',
    'TORREAO': 'TORREAO',
    'ILHA JOANA BEZERRA': 'ILHA JOANA BEZERRA',
    'CAMPO GRANDE': 'CAMPO GRANDE',
    'BOA VISTA': 'BOA VISTA',
    'ILHA DO LEITE': 'ILHA DO LEITE',
    'SANTO AMARO': 'SANTO AMARO',
    'HIPODROMO': 'HIPODROMO',
    'ENCRUZILHADA': 'ENCRUZILHADA',
    'CAJUEIRO': 'CAJUEIRO',
    'CAMPINA DO BARRETO': 'CAMPINA DO BARRETO',
    'CASA AMARELA': 'CASA AMARELA',
    'VARZEA': 'VARZEA',
    'CIDADE UNIVERSITARIA': 'CIDADE UNIVERSITARIA',
    'DOIS IRMAOS': 'DOIS IRMAOS',
    'CASA FORTE': 'CASA FORTE',
    'GUABIRABA': 'GUABIRABA',
    'POÇO DA PANELA': 'POCO', 
    'BREJO DA GUABIRABA': 'BREJO DA GUABIRABA',
    'APIPUCOS': 'APIPUCOS',
    'UR-7 VARZEA': 'VARZEA',
    'MONTEIRO': 'MONTEIRO',
    'CAXANGA': 'CAXANGA',
    'CORREGO DO JENIPAPO': 'CORREGO DO JENIPAPO',
    'BRASILIT': 'AGUA FRIA',
    'NOVA DESCOBERTA': 'NOVA DESCOBERTA',
    'MORRO DA CONCEICAO': 'MORRO DA CONCEICAO',
    'MACAXEIRA': 'MACAXEIRA',
    'VASCO DA GAMA': 'VASCO DA GAMA',
    'TORRE': 'TORRE',
    'MADALENA': 'MADALENA',
    'CORDEIRO': 'CORDEIRO',
    'ZUMBI': 'ZUMBI',
    'IPUTINGA': 'IPUTINGA',
    'ILHA DO RETIRO': 'ILHA DO RETIRO',
    'GRACAS': 'GRACAS',
    'ROSARINHO': 'ROSARINHO',
    'ALTO JOSE DO PINHO': 'ALTO JOSE DO PINHO',
    'ARRUDA': 'ARRUDA',
    'TAMARINEIRA': 'TAMARINEIRA',
    'DERBY': 'DERBY',
    'PARNAMIRIM': 'PARNAMIRIM',
    'ESPINHEIRO': 'ESPINHEIRO',
    'AFLITOS': 'AFLITOS',
    'MANGABEIRA': 'MANGABEIRA',
    'CAPUNGA': 'GRACAS',
    'ALTO JOSE BONIFACIO': 'ALTO JOSE BONIFACIO',
    'AGUA FRIA': 'AGUA FRIA',
    'LINHA DO TIRO': 'LINHA DO TIRO',
    'BEBERIBE': 'BEBERIBE',
    'BOMBA DO HEMETERIO': 'BOMBA DO HEMETERIO',
    'BREJO DE BEBERIBE': 'BREJO DE BEBERIBE',
    'DOIS UNIDOS': 'DOIS UNIDOS',
    'FUNDAO': 'FUNDAO',
    'ALTO SANTA TEREZINHA': 'ALTO SANTA TEREZINHA',
    'PASSARINHO': 'PASSARINHO',
    'PORTO DA MADEIRA': 'PORTO DA MADEIRA',
    'IBURA DE BAIXO': 'IBURA',
    'JORDÃO': 'JORDAO',
    'JORDAO BAIXO': 'JORDAO',
    'JORDAO ALTO': 'JORDAO',
    'ENGENHO DO MEIO': 'ENGENHO DO MEIO',
    'BONGI': 'BONGI',
    'MUSTARDINHA': 'MUSTARDINHA',
    'TORROES': 'TORROES',
    'SAN MARTIN': 'SAN MARTIN'
}